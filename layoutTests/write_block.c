...
extern int wrt_thds;
extern char charColor[];
extern char charGreyColor[];
...
write_block(void* restrict arg)
{
  ...
  for (size_t ib = offset; ib < pic_size; ib += blocksize) {
    ...
    for (size_t ix = ib; ix < ie;) {
      ...
      for (; ix < ie && item_done_loc[ix]; ix += wrt_thds) {
        ...
        for (size_t jx = 0; jx < pic_size; jx++) {
          ...
          for (size_t iz = 0; iz < 12; iz++) {
            attra_char_colors[12 * jx + iz] =
              charGreyColor[4 * color_type + iz % 4];
          }
          // printf("%d %d %d\n",
          //        output_colors[3 * jx],
          //        output_colors[3 * jx + 1],
