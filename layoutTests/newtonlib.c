...
void*
newton1(double x_re, double x_im, int* attractor, int* convergence)
{
  ...
  while (1) {
    if ((x_re * x_re + x_im * x_im) < 0.000001 || // closer than 0.001 to origin
        (fabs(x_re) > 10000000000 ||
         fabs(x_im) >
           10000000000)) { // real or imaginary part larger than 10^10
      *attractor = 9;
      if (iter <= capIter) {
        *convergence = iter;
      } else {
...
void*
newton2(double x_re, double x_im, int* attractor, int* convergence)
{
  ...
  while (1) {
    ...
    if ((delta1_re * delta1_re + delta1_im * delta1_im) <
        0.000001) { // converged to root1
      *attractor = 0;
      if (iter <= capIter) {
        *convergence = iter;
    ...
    if ((delta2_re * delta2_re + delta2_im * delta2_im) <
        0.000001) { // converged to root2
      *attractor = 1;
      if (iter <= capIter) {
        *convergence = iter;
      } else {
...
void*
newton3(double x_re, double x_im, int* attractor, int* convergence)
{
  // Newton routine for polynomial of degree 3
  double r1_re = 1, r1_im = 0, r2_re = -0.5, r2_im = 0.86603, r3_re = -0.5,
         r3_im = -0.86606;
  int iter = 0, capIter = 100;
  ...
  while (1) {
    ...
    if ((delta3_re * delta3_re + delta3_im * delta3_im) <
        0.000001) { // converged to root3
      *attractor = 2;
      if (iter <= capIter) {
        *convergence = iter;
      } else {
...
void*
newton4(double x_re, double x_im, int* attractor, int* convergence)
{
  // Newton routine for polynomial of degree 4
  double r1_re = 1, r1_im = 0, r2_re = 0, r2_im = 1, r3_re = -1, r3_im = 0,
         r4_re = 0, r4_im = -1;
  int iter = 0, capIter = 100;
  ...
  while (1) {
    ...
    if ((delta4_re * delta4_re + delta4_im * delta4_im) <
        0.000001) { // converged to root4
      *attractor = 3;
      if (iter <= capIter) {
        *convergence = iter;
      } else {
...
void*
newton5(double x_re, double x_im, int* attractor, int* convergence)
{
  // Newton routine for polynomial of degree 5
  double r1_re = 1, r1_im = 0, r2_re = 0.309017, r2_im = 0.951057,
         r3_re = -0.809017, r3_im = 0.587785, r4_re = -0.809017,
         r4_im = -0.587785, r5_re = 0.309017, r5_im = -0.951057;
  int iter = 0, capIter = 100;
  ...
  while (1) {
    ...
    if ((delta5_re * delta5_re + delta5_im * delta5_im) <
        0.000001) { // converged to root5
      *attractor = 4;
      if (iter <= capIter) {
        *convergence = iter;
      } else {
...
void*
newton6(double x_re, double x_im, int* attractor, int* convergence)
{
  // Newton routine for polynomial of degree 6
  double r1_re = 1, r1_im = 0, r2_re = 0.5, r2_im = 0.866025, r3_re = -0.5,
         r3_im = 0.866025, r4_re = -1, r4_im = 0, r5_re = -0.5,
         r5_im = -0.866025, r6_re = 0.5, r6_im = -0.866025;
  int iter = 0, capIter = 100;
  ...
  while (1) {
    ...
    double a_re = 0, a_im = 0, b_re = 0, b_im = 0, c_re = 0, c_im = 0, d_re = 0,
           d_im = 0;
    mul_cpx(&a_re, &a_im, x_re, x_im, x_re, x_im); // x^2
    ...
    if ((delta6_re * delta6_re + delta6_im * delta6_im) <
        0.000001) { // converged to root6
      *attractor = 5;
      if (iter <= capIter) {
        *convergence = iter;
      } else {
...
void*
newton7(double x_re, double x_im, int* attractor, int* convergence)
{
  // Newton routine for polynomial of degree 7
  double r1_re = 1, r1_im = 0, r2_re = 0.62349, r2_im = 0.781831,
         r3_re = -0.222521, r3_im = 0.974928, r4_re = -0.900969,
         r4_im = 0.433884, r5_re = -0.900969, r5_im = -0.433884,
         r6_re = -0.222521, r6_im = -0.974928, r7_re = 0.62349,
         r7_im = -0.781831;
  int iter = 0, capIter = 100;
  ...
  while (1) {
    ...
    if ((delta7_re * delta7_re + delta7_im * delta7_im) <
        0.000001) { // converged to root7
      *attractor = 6;
      if (iter <= capIter) {
        *convergence = iter;
      } else {
...
void*
newton8(double x_re, double x_im, int* attractor, int* convergence)
{
  // Newton routine for polynomial of degree 8
  double r1_re = 1, r1_im = 0, r2_re = 0.707107, r2_im = 0.707107, r3_re = 0,
         r3_im = 1, r4_re = -0.707107, r4_im = 0.707107, r5_re = -1, r5_im = 0,
         r6_re = -0.707107, r6_im = -0.707107, r7_re = 0, r7_im = -1,
         r8_re = 0.707107, r8_im = -0.707107;
  int iter = 0, capIter = 100;
  ...
  while (1) {
    ...
    if ((delta8_re * delta8_re + delta8_im * delta8_im) <
        0.000001) { // converged to root8
      *attractor = 7;
      if (iter <= capIter) {
        *convergence = iter;
      } else {
...
void*
newton9(double x_re, double x_im, int* attractor, int* convergence)
{
  // Newton routine for polynomial of degree 9
  double r1_re = 1, r1_im = 0, r2_re = 0.766044, r2_im = 0.642788,
         r3_re = 0.173648, r3_im = 0.984808, r4_re = -0.5, r4_im = 0.866025,
         r5_re = -0.939693, r5_im = 0.34202, r6_re = -0.939693,
         r6_im = -0.34202, r7_re = -0.5, r7_im = -0.866025, r8_re = 0.173648,
         r8_im = -0.984808, r9_re = 0.766044, r9_im = -0.642788;
  int iter = 0, capIter = 100;
  ...
  while (1) {
    ...
    if ((delta9_re * delta9_re + delta9_im * delta9_im) <
        0.000001) { // converged to root9
      *attractor = 8;
      if (iter <= capIter) {
        *convergence = iter;
      } else {
